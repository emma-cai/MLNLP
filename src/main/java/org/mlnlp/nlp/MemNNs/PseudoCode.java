...
class TrainModel {
    ...
    /**
     *
     * @param xs List of instance indices.
     * @param y_t Expected
     * @param L List of I(x) for each instance.
     * @param V VocabularyIndex matrix.
     * @return
     */
    public double sR(List<Integer> xs, int y_t, List<double[]> L, List<double[]> V) {
        ...
        for (int t = 0; t < xs.size(); t++) {
            x_t = xs.get(t);
            if (t == 0) {   // words of the input arguments are from the actual input x
                XPhi = phi_x1(x_t, L);
            } else {        // words of the input arguments are from the supporting memories
                XPhi = phi_x2(x_t, L);
            }
            double[] YPhi = phi_y(y_t, V);
